...
const SignupScreen = ({ navigation }) => {
  ...
  async function loginHandler() {
    ...
    if (response.error) {
      displayMessage(response.payload.message || "Invalid email or password. Please try again.");
      // setMessage(response.payload.message || "Invalid email or password. Please try again.");
      // Animated.timing(messageAnim, {
      //   toValue: 1,
      //   duration: 500,
      //   useNativeDriver: true,
  ...
  async function signupHandler() {
    console.log(instructorEmail, firstName, lastName, instructorPassword);
    const instructorData = { instructorEmail, instructorPassword, firstName, lastName };

    if (!instructorEmail.trim() || !instructorPassword.trim()) {
      displayMessage("Email and password are required.");
      return;
    }
    ...
    const response = await dispatch(signup(instructorData));
    if (response.error) {
      // console.log(response.payload,"response.payload.message");
      displayMessage(response.payload || "Invalid email or password. Please try again.");
    } else {
      setMessage("");
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={[styles.link,{marginTop:0, marginBottom:10, fontWeight:'bold'}]}>as a student</Text>
        <TextInput placeholder="Email" style={styles.input} onChangeText={setInstructorEmail} value={instructorEmail} />
        ...
        <View style={styles.passwordContainer}>
          <TextInput
            ...
            onChangeText={setInstructorPassword}
            value={instructorPassword}
            secureTextEntry={!showInstructorPassword}
          />
          <TouchableOpacity onPress={() => setShowInstructorPassword(!showInstructorPassword)}>
            <Ionicons name={showInstructorPassword ? "eye" : "eye-off"} size={24} color={COLORS.primary} />
          </TouchableOpacity>
        </View>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('InstructorLoginScreen')}>
          <Text style={styles.link}>I am a Instructor</Text>
        </TouchableOpacity>
      </Animated.View>

      <Animated.View 
        style={[styles.card, styles.backCard, { transform: [{ rotateY: backInterpolate }] }]} 
        pointerEvents={!isFlipped ? "none" : "auto"}
      >
